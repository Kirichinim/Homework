...
def btn_finish_click():
    ...
    header.grid_remove()
    result_test = Label(text=f'Вы коммунист на: {count_true_answers} буханок хлеба из {len(qas)}', padx=15, pady=10, fg="red", bg="black", font=("Arial Bold", 25))
    result_test.grid(row=0, column=0, sticky=W)
    messagebox.showinfo("Конец опроса","Вы ответили на возложенные на вас вопросы. Вы можете ознакомиться с результатом сейчас.")
root = Tk()
qas = list()
cur_qa = -1
...
radio_buttons = list()
btn_ok = Button(text="Следующий вопрос", fg="red", bg="black", font=("Arial Bold", 15), command=btn_ok_click)
btn_finish = Button(text="Закончить", fg="red", bg="black", font=("Arial Bold", 15), command=btn_finish_click)
...
def main():
    lines = list()
    with open("VOPROSI.txt", mode="r", encoding="utf-8") as file_in:
        for line in file_in:
    ...
    root.title("Коммунистический опрос")
